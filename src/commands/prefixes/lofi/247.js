...
const { hasVoted } = require("../../../lib/topgg");
...
module.exports = {
  ...
  async execute(message) {
    ...
    // Turning it ON is voter-gated; owners bypass. Turning it OFF is always allowed.
    if (next) {
      const isOwner = message.client.config.ownerID.includes(message.member.user.id);
      const allowed = isOwner || (await hasVoted(message.member.user.id));

      if (!allowed) {
        const voteLink = hyperlink(bold("Vote on Top.gg"), message.client.config.topgg.voteUrl);
        return message.replyWithoutMention({
          embeds: [errorEmbed(`24/7 mode is available for voters. Please ${voteLink} first, then run this command again. Votes refresh every 12 hours.`)],
        });
      }
    }
