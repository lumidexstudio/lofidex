...
import { hasVoted } from "../../../lib/topgg";
import type { MessageWithReply } from "../../../types";

export = {
  name: "247",
  description:
    "Keep the bot in the voice channel even when it's empty. Enabling requires a top.gg vote.",
  aliases: ["24/7", "stay"],
  cooldown: 5,
  category: "lofi",
  async execute(
    ...
    if (next) {
      const isOwner = message.client.config.ownerID.includes(
        message.member!.user.id
      );
      const allowed =
        isOwner || (await hasVoted(message.member!.user.id));

      if (!allowed) {
        const voteLink = hyperlink(
          bold("Vote on Top.gg"),
          message.client.config.topgg.voteUrl
        );
        return message.replyWithoutMention!({
          embeds: [
            errorEmbed(
              `24/7 mode is available for voters. Please ${voteLink} first, then run this command again. Votes refresh every 12 hours.`
            ),
          ],
        });
      }
